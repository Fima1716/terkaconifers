...
const REGION_RULES: [string, RegExp[]][] = [
  ...
  ['Санкт-Петербург', [/^санкт-петербург/i, /^спб[, ]/i, /^с-петербург/i]],
  ['Ленинградская область', [/ленинград/i, /^ло[, ]/i, /петербург/i, /гатчин/i, /тоснен/i, /всеволож/i, /(?<!ка)лужск/i, /лисино/i]],
...
function parseGarden(garden: string): { display: string; type: string; type_ru: string } {
  ...
  let display = garden
    ...
    // Союз "И" слипшийся с именем: "Сергея ИОльги" → "Сергея И Ольги"
    .replace(/(^|\s)И([А-ЯЁ][а-яё])/g, '$1И $2')
